...
4. [Device management](#4-device-management)
   - [4.1 Step-by-step: register a device individually](#41-step-by-step-register-a-device-individually)
   - [4.2 Step-by-step: bulk import devices](#42-step-by-step-bulk-import-devices)
   - [4.3 Step-by-step: find and filter devices](#43-step-by-step-find-and-filter-devices)
   - [4.4 Step-by-step: request an edit to a device](#44-step-by-step-request-an-edit-to-a-device)
   - [4.5 Step-by-step: request a delete](#45-step-by-step-request-a-delete)
   - [4.6 Step-by-step: request a device status change](#46-step-by-step-request-a-device-status-change)
5. [Sub distributions & the confirmation workflow](#5-sub-distributions--the-confirmation-workflow)
   - [5.1 How the workflow works (read this first)](#51-how-the-workflow-works-read-this-first)
   - [5.2 Step-by-step: create a distribution individually](#52-step-by-step-create-a-distribution-individually)
   - [5.3 Step-by-step: bulk upload a distribution](#53-step-by-step-bulk-upload-a-distribution)
   - [5.4 Step-by-step: view and search distributions](#54-step-by-step-view-and-search-distributions)
   - [5.5 Downloading the device list of a distribution](#55-downloading-the-device-list-of-a-distribution)
   - [5.6 The statuses, explained](#56-the-statuses-explained)
   - [5.7 Handling disputes and the confirmation flow](#57-handling-disputes-and-the-confirmation-flow)
   - [5.8 Where distributions appear in Approvals](#58-where-distributions-appear-in-approvals)
6. [Defects, replacements & returns](#6-defects-replacements--returns)
   - [6.1 The defect workflow (read this first)](#61-the-defect-workflow-read-this-first)
   - [6.2 The Defect Reports page (how to check it)](#62-the-defect-reports-page-how-to-check-it)
   - [6.3 Step-by-step: review and handle a defect report](#63-step-by-step-review-and-handle-a-defect-report)
   - [6.4 Step-by-step: replace a defective device (in detail)](#64-step-by-step-replace-a-defective-device-in-detail)
   - [6.5 The Return Requests page, in detail](#65-the-return-requests-page-in-detail)
   - [6.6 The Pending Dues page, in detail](#66-the-pending-dues-page-in-detail)
   - [6.7 The Approvals page, in detail](#67-the-approvals-page-in-detail)
7. [External inventory (items & distribution)](#7-external-inventory-items--distribution)
   - [7.1 What external inventory is (read this first)](#71-what-external-inventory-is-read-this-first)
   - [7.2 The page layout](#72-the-page-layout)
   - [7.3 Step-by-step: register an item individually](#73-step-by-step-register-an-item-individually)
   - [7.4 Step-by-step: edit an item](#74-step-by-step-edit-an-item)
   - [7.5 Step-by-step: delete an item](#75-step-by-step-delete-an-item)
   - [7.6 Step-by-step: bulk upload items from CSV](#76-step-by-step-bulk-upload-items-from-csv)
   - [7.7 Step-by-step: distribute a single item](#77-step-by-step-distribute-a-single-item)
   - [7.8 Step-by-step: bulk distribution from a file](#78-step-by-step-bulk-distribution-from-a-file)
   - [7.9 The Distributed Items page (history)](#79-the-distributed-items-page-history)
   - [7.10 Warranty status, explained](#710-warranty-status-explained)
   - [7.11 What you can do](#711-what-you-can-do)
...
> **Want to change your login email or reset your own password?** Your profile lets you
> do both. If you ever need a different account credential changed, submit a
> **change request** (section 4) so the Manager or Super Admin can apply it.
...
where it is and everywhere it has been. It appears in your sidebar; as PDIC Staff you
...
As PDIC Staff you can **register** devices (one at a time or in bulk) and **view every
device**. Changes to existing devices — **edits, deletes, and status changes** — are
submitted as **requests** and applied only after a Manager or Super Admin approves them.

> **The golden rule:** think of your write actions on devices as "proposals". Registering
> and importing are immediate; everything that changes an existing device waits for a
> Manager or Super Admin. This is the key difference between you and the Manager, whose
> edits and deletes apply instantly.

### 4.1 Step-by-step: register a device individually
...
### 4.2 Step-by-step: bulk import devices
...
### 4.3 Step-by-step: find and filter devices

1. Open **Devices**. As PDIC Staff the page shows **"All registered devices in the
...
6. From Device Details you can **Edit Device** (sends a request, see 4.4), **Track
   Device** (jumps to the Track page with this device pre-filled), or see the
   **Replacement Mapping** box linking a defective device to its replacement.
...
2. Click the **amber pencil (Edit)** icon on its row — or open Device Details and click
   **Edit Device**.
3. The modal is titled **"Request Device Edit (Approval Required)"** and shows an amber
   notice explaining that your changes need approval.
4. Make the changes: **Device Type, Band/Box Type, Serial Number, MAC Address, Model,
   NUID, Vendor,** and **Location**.
5. Click **Submit for Approval**.
6. A green "Edit request submitted. A manager or admin will review your proposed
   changes." message confirms it.

**What happens next:** the request appears in the Manager/Super Admin's queue. Once
approved, the device is updated with your proposed values; if rejected, the device
stays as it was. You are notified either way.
...
1. In **Devices**, tick the **checkboxes** on the rows you want to remove (or the box in
   the header to select the whole page).
2. A blue panel appears at the bottom: "*N device(s) selected*" with a
   **Delete Selected** button.
3. Click **Delete Selected**. The dialog is titled **"Request Device Deletion"** and
   asks for a **Reason** — explain why these devices should be deleted (this is
   required for staff requests).
4. Click **Submit Request**.

**What happens next:** the request (with the device IDs and your reason) goes to the
Manager or Super Admin. When approved, the listed devices are deleted; if rejected,
nothing changes. Check the request carefully before submitting — a mistaken bulk delete
has no undo, and approval is up to the Manager.
...
3. Pick the **requested status**, add a **reason** for the change.
4. Submit. You'll see "Status change request submitted. Awaiting admin/manager
   approval."

**What happens next:** a Manager or Super Admin reviews it in the change-request queue.
On approval the device status is set; on rejection it stays as it was.

> **Why this matters:** these approval steps protect the device database. Your requests
> are the raw material the management team acts on — so write a clear, truthful reason
> for every edit, delete, or status change you submit.

---

## 5. Sub distributions & the confirmation workflow
...
### 5.1 How the workflow works (read this first)
...
5. Under **Select Recipient**:
   - Pick the **Recipient Type** — as PDIC Staff you can send to a **Sub Distributor**,
     **Cluster**, or **Operator**.
...
### 5.3 Step-by-step: bulk upload a distribution
...
### 5.4 Step-by-step: view and search distributions
...
### 5.5 Downloading the device list of a distribution
...
### 5.7 Handling disputes and the confirmation flow
...
### 5.8 Where distributions appear in Approvals
...
that feed are the ones awaiting receipt confirmation. As PDIC Staff you can open any
...
## 6. Defects, replacements & returns
...
### 6.1 The defect workflow (read this first)
...
### 6.2 The Defect Reports page (how to check it)

Open **Defect Reports** from the sidebar. As PDIC Staff you see **every** report in
the system.
...
### 6.3 Step-by-step: review and handle a defect report
...
### 6.4 Step-by-step: replace a defective device (in detail)
...
### 6.5 The Return Requests page, in detail
...
### 6.6 The Pending Dues page, in detail
...
### 6.7 The Approvals page, in detail

**Purpose:** Approvals is the **single merged feed** of everything waiting for your
action. It combines distributions, return confirmations, and defect items so you don't
have to chase each page separately.
...
> **Note:** device **change requests** (edits, deletes, status changes) are **not** part
> of this feed — those are the ones you submit in section 4, and they are reviewed by
> the Manager or Super Admin in their change-request queue.
...
## 7. External inventory (items & distribution)

### 7.1 What external inventory is (read this first)

External inventory is a **separate catalog from the main Devices section**. Where the
Devices section (section 4) tracks every individual PDIC device with its serial, MAC and
...
- Main-device distributions go through the **confirmation workflow** (section 5).
...
Open **External Inventory** from the sidebar. Management gets a dropdown with four
pages:
...
### 7.3 Step-by-step: register an item individually
...
### 7.5 Step-by-step: delete an item
...
### 7.6 Step-by-step: bulk upload items from CSV
...
2. Open it, replace the samples with your items, and keep the header. The required column
   ...
   `warranty_duration` (months), and `notes`. The same rules as 7.3 apply: an identifier
...
### 7.7 Step-by-step: distribute a single item
...
### 7.8 Step-by-step: bulk distribution from a file
...
### 7.9 The Distributed Items page (history)
...
### 7.10 Warranty status, explained
