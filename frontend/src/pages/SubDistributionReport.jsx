...
const SubDistributionReport = () => {
  ...
  const searchedRows = useMemo(() => {
    ...
    if (by !== 'all' && searchableFields[by]) {
      return rows.filter(r => String(searchableFields[by](r) || '').toLowerCase().includes(query));
    }
    return rows.filter(r => Object.values(searchableFields).some(getter => String(getter(r) || '').toLowerCase().includes(query)));
  }, [rows, appliedSearch]);
  ...
  useEffect(() => { setPage(1); }, [activeView, appliedSearch]);
    ...
    try {
      ...
      if (activeView === 'total') {
        columns = [
          ...
          { header: 'Total Clusters', key: 'total_clusters' },
      ...
      exportTablePdf({
        title: `Sub Distribution Report — ${viewLabel}`,
        subtitle: filterNotes.join(' · ') || undefined,
        columns,
        rows: viewRows,
        filename: `sub-distribution-report-${activeView}`,
      });
    ...
    <Card>
      {searchedRows.length === 0 ? (
        <div className="text-center py-12 text-gray-400">
          ...
          <p>No sub-distribution data found</p>
        </div>
      ) : (
        <>
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-gray-200">
                ...
                <th className={thClass}>Total Clusters</th>
            ...
            <tbody className="divide-y divide-gray-100">
              {pagedRows.map((r, idx) => (
                <tr key={r.sub_id} className="hover:bg-gray-50">
                  ...
                  <td className={`${tdClass} font-medium text-gray-800`}>{r.sub_name}</td>
                  ...
                  <td className={tdClass}>{r.total_clusters || 0}</td>
      ...
      return (
        <Card>
          <div className="text-center py-12 text-gray-400">
            ...
            <p>No sub-distributions have {kind === 'sb' ? 'a digital ID' : 'a broadband ID'}</p>
          </div>
        </Card>
      );
    }
...
export default SubDistributionReport;
