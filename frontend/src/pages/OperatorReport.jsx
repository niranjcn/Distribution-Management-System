...
const OperatorReport = () => {
  ...
  const clusterOptions = useMemo(() => {
    const map = new Map();
    subRows.forEach(r => {
      if (r.cluster_id != null) map.set(String(r.cluster_id), r.cluster_name);
    });
    return [...map.entries()]
      .sort((a, b) => String(a[1]).localeCompare(String(b[1])))
      .map(([id, name]) => ({ id, name }));
  ...
  const filteredRows = useMemo(() => {
    if (!clusterFilter) return subRows;
    return subRows.filter(r => String(r.cluster_id) === String(clusterFilter));
  }, [subRows, clusterFilter]);
  ...
  useEffect(() => { setPage(1); }, [activeView, subFilter, clusterFilter, appliedSearch]);
  ...
  const handleDownloadPdf = () => {
    try {
      ...
      if (clusterFilter) filterNotes.push(`Cluster: ${clusterOptions.find(o => o.id === clusterFilter)?.name || clusterFilter}`);
      const rangeLabel = dateRange?.range;
      if (rangeLabel && rangeLabel !== 'all') filterNotes.push(`Date range: ${rangeLabel}`);

      exportTablePdf({
        title: `Operator Report — ${viewLabel}`,
        subtitle: filterNotes.join(' · ') || undefined,
        columns,
        rows: viewRows,
        filename: `operator-report-${activeView}`,
      });
      ...
      return (
        <Card>
          <div className="text-center py-12 text-gray-400">
            ...
            <p>No operators have {kind === 'sb' ? 'a digital ID' : 'a broadband ID'}</p>
          </div>
        </Card>
      );
    }
              ...
              {pagedRows.map((r, idx) => {
                ...
                return (
                  <tr key={r.operator_id} className="hover:bg-gray-50">
                    ...
                    <td className={tdClass}>{r.operator_name}</td>
                    <td className={tdClass}>{r.sub_name || '—'}</td>
                    <td className={tdClass}>{r.cluster_name || '—'}</td>
                    {vendors.map(v => (
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        <label className={`text-sm font-medium ${subFilter ? 'text-gray-600' : 'text-gray-400'}`}>Cluster</label>
        <select
          value={clusterFilter}
          onChange={(e) => setClusterFilter(e.target.value)}
          disabled={!subFilter}
          className={`${selectClass} ${subFilter ? '' : 'bg-gray-50 text-gray-400 cursor-not-allowed'}`}
        >
          <option value="">All</option>
